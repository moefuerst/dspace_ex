...
defimpl DSpace.API.Operation, for: DSpace.API.Operation.JSON do
  ...
  def perform(operation, client, opts_override) do
    ...
    headers =
      ...
      |> maybe_add_csrf_header(resolve_csrf(operation.csrf, operation.http_method), client.csrf_token)
      |> maybe_add_csrf_cookie(client.csrf_token)
  ...
  defp maybe_invoke_callback(%{before_step: callback} = operation, client, options) do
  ...
  defp maybe_add_csrf_cookie(headers, csrf_token) when is_nonempty_binary(csrf_token) do
  ...
  defp maybe_add_csrf_cookie(headers, _csrf_token), do: headers
  ...
  defp maybe_invoke_on_response_hook(%API{on_response_hook: nil}, _response), do: :ok
  ...
  defp apply_transformer(response, transformer) when is_function(transformer, 1) do
    transformer.(response)
  rescue
    exception ->
      Error.response_validation_error(response, Exception.message(exception))
  end
end

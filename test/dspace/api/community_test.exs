defmodule DSpace.API.CommunityTest do
  ...
  describe "retrieving a community by UUID" do
    test "returns community when it exists", %{bypass: bypass, api: api} do
      uuid = "7669c72a-3f2a-451f-a3b9-9210e7a4c02f"
      community_fixture = load_fixture("fetch_community.json")

      Bypass.expect_once(bypass, "GET", "/api/core/communities/#{uuid}", fn conn ->
  ...
  describe "retrieving multiple communities" do
    test "lists communities with pagination support", %{bypass: bypass, api: api} do
      communities_fixture = load_fixture("fetch_communities.json")

      Bypass.expect_once(bypass, "GET", "/api/core/communities", fn conn ->
    ...
    test "retrieves top-level communities", %{bypass: bypass, api: api} do
      communities_fixture = load_fixture("fetch_communities.json")

      Bypass.expect_once(bypass, "GET", "/api/core/communities/search/top", fn conn ->
        params = Plug.Conn.fetch_query_params(conn).query_params
  ...
  describe "searching for communities" do
    test "finds all communities when no search criteria provided", %{bypass: bypass, api: api} do
    ...
    test "searches communities by query term", %{bypass: bypass, api: api} do
  ...
  describe "creating communities" do
    test "creates top-level community without parent", %{bypass: bypass, api: api} do
    ...
    test "creates subcommunity with parent", %{bypass: bypass, api: api} do
  ...
  describe "modifying existing communities" do
    test "updates community with patch operations", %{bypass: bypass, api: api} do
      uuid = "7669c72a-3f2a-451f-a3b9-9210e7a4c02f"
    ...
    test "replaces entire community content", %{bypass: bypass, api: api} do
      uuid = "7669c72a-3f2a-451f-a3b9-9210e7a4c02f"
  ...
  describe "removing communities" do
    test "deletes community successfully", %{bypass: bypass, api: api} do
      uuid = "7669c72a-3f2a-451f-a3b9-9210e7a4c02f"

      Bypass.expect_once(bypass, "DELETE", "/api/core/communities/#{uuid}", fn conn ->
  ...
  describe "retrieving related resources" do
    test "retrieves collections within community", %{bypass: bypass, api: api} do
      uuid = "community-uuid"
      collections_fixture = load_fixture("fetch_collections.json")

      Bypass.expect_once(bypass, "GET", "/api/core/communities/#{uuid}/collections", fn conn ->
        params = Plug.Conn.fetch_query_params(conn).query_params
        assert params["page"] == "1"
        assert params["size"] == "5"
    ...
    test "retrieves subcommunities within community", %{bypass: bypass, api: api} do
      uuid = "community-uuid"
      communities_fixture = load_fixture("fetch_communities.json")

      Bypass.expect_once(bypass, "GET", "/api/core/communities/#{uuid}/subcommunities", fn conn ->
        params = Plug.Conn.fetch_query_params(conn).query_params
        assert params["page"] == "2"
    ...
    test "retrieves parent community", %{bypass: bypass, api: api} do
    ...
    test "handles missing parent community", %{bypass: bypass, api: api} do
